    ...
    .then(savedUser=>{
        ...
        .then(doMatch=>{
            if(doMatch){
               ...
               const token = jwt.sign({_id:savedUser._id},JWT_SECRET)//user id and the secret key
               const {_id,name,email,followers,following,pic} = savedUser//if successful authentication we will provide these 4 items
               res.json({token,user:{_id,name,email,followers,following,pic}})//json response
            }
            else{
                return res.status(422).json({error:"Invalid Email or password"})
            }
